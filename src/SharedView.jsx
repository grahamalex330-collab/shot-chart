...
import { ZONES, THREE_PT, mapZone, toEventLog, computeGameStats } from "./gameEngine.js";
...
export default function SharedView({ gameId }) {
  ...
  const y3a = Object.entries(st.players).reduce((s,[,v])=>s+v.p3a,0);
  const y3m = Object.entries(st.players).reduce((s,[,v])=>s+v.p3m,0);
  const y3p = y3a>0?Math.round(y3m/y3a*100):0;
  const sortedPlayers = [...players].sort((a,b)=>parseInt(a.number)-parseInt(b.number));
  const zoneStats = st.zoneStats;
  ...
  return (
    <div style={SHELL}>
      ...
      <div style={{textAlign:"center",padding:"16px 0 12px"}}><div style={{fontSize:56,fontWeight:900,color:"#facc15",lineHeight:1}}>{st.totalPts}</div><div style={{fontSize:11,color:"#666",letterSpacing:2,marginTop:4}}>TOTAL POINTS</div></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:6,padding:"0 16px 8px"}}>
        ...
        <div style={{textAlign:"center",background:"rgba(255,255,255,0.03)",borderRadius:10,padding:"10px 4px"}}><div style={{fontSize:18,fontWeight:800,color:y3p>=35?"#22c55e":y3p>=25?"#facc15":y3a?"#ef4444":"#555"}}>{y3a?y3p+"%":"—"}</div><div style={{fontSize:8,color:"#666"}}>3PT {y3m}/{y3a}</div></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:6,padding:"0 16px 16px"}}>
        ...
        <div style={{textAlign:"center",background:"rgba(255,255,255,0.03)",borderRadius:10,padding:"8px 4px"}}><div style={{fontSize:16,fontWeight:800,color:"#22c55e"}}>{st.teamReb}</div><div style={{fontSize:8,color:"#666"}}>REB</div></div>
        <div style={{textAlign:"center",background:"rgba(255,255,255,0.03)",borderRadius:10,padding:"8px 4px"}}><div style={{fontSize:16,fontWeight:800,color:"#a855f7"}}>{st.teamTOs}</div><div style={{fontSize:8,color:"#666"}}>TO</div></div>
      ...
      {st.oppOrbTotal>0&&<div style={{display:"flex",justifyContent:"center",padding:"0 16px 12px"}}><div style={{background:"rgba(239,68,68,0.06)",border:"1px solid rgba(239,68,68,0.15)",borderRadius:8,padding:"6px 16px",fontSize:12}}><span style={{color:"#ef4444",fontWeight:700}}>OPP ORB: {st.oppOrbTotal}</span></div></div>}
      ...
      <div style={{padding:"0 16px",margin:"0 8px"}}>
        <svg viewBox="0 0 400 320" style={{width:"100%",height:"auto",display:"block"}}>
          ...
          {ZONES.map(z=>{const s=zoneStats[z.id];return(
            <g key={z.id}><rect x={z.x} y={z.y} width={z.w} height={z.h} rx="8" fill={getZoneColor(z.id)} stroke={getZoneBorder(z.id)} strokeWidth={2} />
            {s&&s.total>0?<><text x={z.cx} y={z.cy-2} textAnchor="middle" fill={getZoneText(z.id)} fontSize="14" fontWeight="800">{z.label}</text><text x={z.cx} y={z.cy+14} textAnchor="middle" fill="#fff" fontSize="12" fontWeight="600">{s.makes}/{s.total}</text></>
            :<text x={z.cx} y={z.cy+5} textAnchor="middle" fill="rgba(255,255,255,0.2)" fontSize="12">{z.label}</text>}
            </g>);})}
        </svg>
      </div>
      ...
      {sortedPlayers.length>0&&<div style={{padding:"16px 16px 4px"}}><div style={SECHEAD}>Player stats</div>
        {sortedPlayers.map(p=>{
          ...
          const pFGp = ps.fga>0?Math.round(ps.fgm/ps.fga*100):0;
          const foulColor = ps.fouls>=4?"#ef4444":ps.fouls>=3?"#f97316":"#666";
          const parts=[]; parts.push("FG "+ps.fgm+"/"+ps.fga+(ps.fga>0?" ("+pFGp+"%)":""));
          if(ps.p3a>0)parts.push("3PT "+ps.p3m+"/"+ps.p3a); if(ps.fta>0)parts.push("FT "+ps.ftm+"/"+ps.fta);
          if(ps.ast>0)parts.push(ps.ast+" ast"); if(ps.reb>0)parts.push(ps.reb+" reb"); if(ps.stl>0)parts.push(ps.stl+" stl"); if(ps.blk>0)parts.push(ps.blk+" blk");
          ...
          return <div key={p.number} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              {ps.fouls>0&&<div style={{textAlign:"center"}}><div style={{fontSize:15,fontWeight:800,color:foulColor}}>{ps.fouls}</div><div style={{fontSize:7,color:foulColor}}>{ps.fouls>=4?"⚠ FOUL":"FOUL"}</div></div>}
